      ...
      <el-table :data="userAuditlist" border highlight-current-row element-loading-text="拼命加载中" v-loading="listLoading"
        ...
        <el-table-column fixed="right" label="操作" width="320">
          <template slot-scope="scope">
            ...
            <el-button :disabled="isBtnTg(scope.row)" size="mini" type="primary" @click="getThrough(scope.row)">通过
            </el-button>
            <el-button :disabled="isBtnTg(scope.row)" size="mini" type="danger" @click="reject(scope.row)">拒绝
            </el-button>
            <el-button :disabled="isBtnTg(scope.row)" size="mini" type="warning" plain @click="repulse(scope.row)">
              打回
            </el-button>
          </template>
        </el-table-column>
...
//          inviteCode: this.search.inviteCode,
      ...
      //打回
      repulse(row) {//claimCustomer
        this.$prompt("打回的原因", "打回", {
          ...
          if (value == null) {
            ...
            Api.testApi.repulseCus(para).then(res => {
              if (res.code == "0000") {
                this.$message({
                  message: "操作成功!",
                  type: "success"
      ...
      //通过
      getThrough(row) {
        this.$confirm('确认通过吗?', '提示', {type: 'success'}).then(() => {
          let para = {
            cusId: row.cusId,
            auditStatus: 4,
          };
          Api.testApi.auditCustomer(para).then(res => {
            if (res.code == '0000') {
              this.$message({
                message: '操作成功',
                type: 'success'
